    ...
    server: {
      ...
      proxy: {
        '/apiLLM': {
          ...
          changeOrigin: true,
          // ws: true,
          rewrite: path => path.replace(/^\/apiLLM/, ''),
        },
        '/apiNormalize': {
          target: 'http://10.0.1.168:25640',
          changeOrigin: true,
          // ws: true,
          rewrite: path => path.replace(/^\/apiNormalize/, ''),
        },
        '/apiNER': {
          target: 'http://10.0.3.73:22890',
          changeOrigin: true,
          // ws: true,
          rewrite: path => path.replace(/^\/apiNER/, ''),
        },
        '/apiToken': {
          ...
          rewrite: path => path.replace(/^\/apiToken/, ''),
        },
      },
    },
