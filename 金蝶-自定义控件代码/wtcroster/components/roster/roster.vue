<template>
    <div class="wtc_roster-wrap">
        ...
        <div class="wtc_roster-table">
            ...
            <rosterTable_org v-if="rosterData.rosterview === 'taborg'" ref="rosterTable_org" :rosterData="rosterData"
                :style="{'height': showTotal ? 'calc(100% - 118px)' : 'calc(100% - 50px)'}"
                @tableLineSlide="tableLineSlide">
            </rosterTable_org>
            ...
            <div v-if="rosterData.type === 1" class="wtc_roster-table-fold-btn"
...
<script>
    ...
    export default {
        name: "roster",
        components: { tableHead, rosterTable, shiftDialog, slideBar, total, rosterTable_org },
        props: ['rosterData'],
        inject: ['model', 'KDApi'],
        ...
        methods: {
            ...
            * */
            infoDetailWidthChange(newWidth) {
                ...
                this.$refs.slideBar.rightWidth = `calc(100% - 230px - 40px - ${newWidth}px)`;
                // 统计栏部分宽度变化（预览模块没有统计栏）
                if (this.$refs.total) this.$refs.total.middleWidth = newWidth + 'px';
                if (this.$refs.total) this.$refs.total.rightWidth = `calc(100% - 230px - 40px - ${newWidth}px)`;
            },
...
<style scoped>
    ...
    .wtc_roster-wrap .wtc_roster-table {
        position: relative;
        overflow: hidden;
    ...
    /*展开按钮*/
    .wtc_roster-table .wtc_roster-table-fold-btn {
        position: absolute;
        bottom: 0px;
        left: 50%;
    ...
    .wtc_roster-table .wtc_roster-table-fold-btn i {
        margin-left: 4px;
        vertical-align: sub;
    }

    .wtc_roster-table .wtc_roster-table-fold-btn:hover {
        opacity: 0.8;
    }
    ...
    .wtc_roster-wrap .wtc_roster-table .wtc_roster-size-large {
        flex-basis: 106px;
        width: 106px;
    }

    .wtc_roster-wrap .wtc_roster-table .wtc_roster-size-middle {
        flex-basis: 80px;
        width: 80px;
    }

    .wtc_roster-wrap .wtc_roster-table .wtc_roster-size-small {
        flex-basis: 50px;
        width: 50px;
    }
</style>