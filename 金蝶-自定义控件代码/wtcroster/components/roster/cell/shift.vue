<template>
    <div class="wtc_roster-table-right-shift" @mouseenter="showMoreIcon" @mouseleave="hideMoreIcon"
        :shiftid="shiftModel.id"
        :style="{'background-color': bgColor, 'border-left': '2px solid ' + shiftModel.shiftColor}">
        <div v-show="dateAttribute" class="wtc_roster-table-right-shift-icon" :style="{'color': shiftModel.shiftColor}">
            <i class="kdfont" :class="shiftIcon"></i>
        </div>
        <div class="wtc_roster-table-right-shift-name" :title="shiftModel.name"
            :style="{'color': shiftModel.shiftColor}">{{shiftModel.name}}</div>
        <div v-show="showMoreBtn" class="wtc_roster-table-right-shift-more-btn kdfont kdfont-gengduo3"
            @mouseenter.stop.prevent.self="showShiftDetail" @mouseleave.stop.prevent.self="hideShiftDetail"></div>
        <div class="wtc_roster-table-right-shift-lock-icon kdfont kdfont-suo16"></div>
    </div>
</template>
...
<script>
    export default {
        name: "shift",
        props: ['shiftId', 'dateAttribute', 'lockPower', 'cellType'],
        ...
        watch: {
            ...
            dateAttribute(newValue) {
                // 设置班次的图标类型：
                switch (newValue) {
                    case 'WORKDAY':
                        this.shiftIcon = 'kdfont-gongzuori2';
                        break;
                    case 'OFFDAY':
                        this.shiftIcon = 'kdfont-xiuxiri';
                        break;
                    case 'HOLIDAY':
                        this.shiftIcon = 'kdfont-jiaqi2';
                        break;
                    default:
                        this.shiftIcon = 'kdfont-gongzuori2'
                }
            ...
            showShiftDetail(event) {
                ...
                setTimeout(function () {
                    if ($this.myTool.popover_isEnter && !document.querySelector('.wtc_kztTool_popover-wrap')) {
                        ...
                        let str = `
                        <div class="wtc_roster-table-right-shift-detail-wrap" style="min-width: 190px;max-width:400px;overflow: hidden;font-size: 12px;">
                            <div class="wtc_roster-table-right-shift-detail-title-wrap" style="padding-bottom: 5px;display: flex;color: #333;">
                                <div class="wtc_roster-table-right-shift-detail-number" title="${$this.shiftModel.number}" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;padding-right: 0px;padding-top: 4px;max-width: 80px;">${$this.shiftModel.number || '无'}</div>
                                <div class="wtc_roster-table-right-shift-detail-line" style="padding: 4px 4px 0 4px;">|</div>
                                <div class="wtc_roster-table-right-shift-detail-off" style="padding-top: 4px;flex:0;white-space: nowrap;">${$this.shiftModel.off ? 'OFF班' : '非OFF班'}</div>
                                <div class="wtc_roster-table-right-shift-detail-line" style="padding: 4px 4px 0 4px;">|</div>
                                <div class="wtc_roster-table-right-shift-detail-title" title="${$this.shiftModel.name}" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;padding-right: 10px;padding-top: 4px;max-width: 280px;">${$this.shiftModel.name}</div>
                            </div>
                            <div class="wtc_roster-table-right-shift-detail-second" style="${$this.cellType === 'org' ? 'display:none;' : ''} padding: 5px 0;color: #666;">
                            ...
                            <ul class="wtc_roster-table-shift-detail-time-list" style="padding: 5px 0;color: #999;max-height: 200px;overflow-y: auto;overflow-x: hidden;">`;
                        ...
                        str += `
                            <div class="wtc_roster-table-shift-detail-warn" style="color: #FF981D;">${abnormalText}</div>
                        </div>
                    `;
...
<style scoped>
    ...
    .wtc_roster-table-right-shift .wtc_roster-table-right-shift-icon {
        margin-left: 6px;
        flex: 0 0 20px;
        width: 20px;
        height: 20px;
    ...
    .wtc_roster-table-right-shift .wtc_roster-table-right-shift-icon i {
        font-size: 18px;
    }

    .wtc_roster-table-right-shift .wtc_roster-table-right-shift-name {
        /*color: green;*/
        overflow: hidden;
        white-space: nowrap;
    ...
    .wtc_roster-table-right-group-list .wtc_roster-table-right-shift .wtc_roster-table-right-shift-more-btn {
        position: absolute;
        top: -2px;
        right: 2px;
        color: #999;
    ...
    .wtc_roster-table-right-shift .wtc_roster-table-right-shift-more-btn:hover {
        z-index: 3;
    }

    .wtc_roster-table-right-shift .wtc_roster-table-right-shift-lock-icon {
        display: none;
        position: absolute;
        bottom: -1px;
        right: 2px;
    ...
    .wtc_roster-shift-lock .wtc_roster-table-right-shift-icon {
        opacity: 0.5;
    }

    .wtc_roster-shift-lock .wtc_roster-table-right-shift-name {
        opacity: 0.5;
    }

    .wtc_roster-shift-lock .wtc_roster-table-right-shift-lock-icon {
        display: block;
    }
</style>