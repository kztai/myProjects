<template>
    <li class="wtc_roster-table-right-roster-wrap">
        <!--:class="{'wtc_roster-table-right-plan-roster-wrap': cellType === 'plan', 'wtc_roster-table-right-actual-roster-wrap': cellType === 'actual'}">-->
        <div class="wtc_roster-table-right-roster" :class="{
                    'wtc_roster-shift-frozen': cellType === 'orgPerson' ? false : isFrozen,
                    'wtc_roster-table-roster-active': active,
                    'wtc_roster-shift-overlap-red': overlap,
                    'wtc_roster-shift-overlap-red-next': overlapNext,
                    'wtc_roster-shift-lock': cellRoster.lock,
                }" ref="rightRoster" @mouseenter.stop.prevent.self="showDisableReason"
            @mouseleave.stop.prevent.self="hideDisableReason" @mousedown="rosterCellClick" :rosterId="cellRoster.id">

            <div class="wtc_roster-table-right-shift-frozen-wrap"></div>
            <shift v-if="cellRoster.shiftId" :shiftId="cellRoster.shiftId" :dateAttribute="cellRoster.dateAttribute"
                :lockPower="lockPower" :cellType="cellType"></shift>
        </div>
    </li>
</template>
        ...
        methods: {
            ...
            * */
            rosterCellClick(event) {
                ...
                // 先判断班次弹窗是否打开：
                if (this.$root.$refs.index.$refs.roster.showShiftDialog) {
                    // todo 班次弹窗逻辑待完善：
                    // let $dom_shiftSelected = $('.wtc_personnelSchedule-dialog-shift-item-active', $this.rootDom);
                    // if ($dom_shiftSelected[0]) { // 存在选中的班次
                    //     // 当前单元格高亮：
                    //     $dom_activeCell.removeClass('wtc_roster-table-roster-active');
                    //     $dom_curCell.addClass('wtc_roster-table-roster-active');
                    //     // 黏贴班次：
...
<style scoped>
    .wtc_roster-table-right-roster-wrap {
        flex: 1;
        height: 100%;
        min-width: 150px;
    ...
    .wtc_roster-table-roster-active {
        border: 1px solid var(--themeColor, #276FF5);
    }

    .wtc_roster-table-right-roster .wtc_roster-table-right-shift-frozen-wrap {
        display: none;
        position: absolute;
        top: 0;
        left: 0;
    ...
    .wtc_roster-shift-frozen .wtc_roster-table-right-shift-frozen-wrap {
        display: block;
    }
