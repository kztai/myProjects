<template>
    <ul ref="rightGroupList" class="wtc_roster-simple-table-right-group-list" :style="{'width': rightWidth}">
        <li ref="dataItem" class="wtc_roster-simple-table-right-data-item"
            v-for="(rosterData, index) in allRosterData.rosterShiftModelList" :key="index"
            :id="rosterData.rosterPerson.id" :attPersonId="rosterData.rosterPerson.attPersonId">
            <ul class="wtc_roster-simple-table-right-plan-list"
                ...
                <cell @showRightClickMenu="showRightClickMenu" @shiftActiveCells="shiftActiveCells"
                    @cancelAllActive="cancelAllActive" ref="cells"
                    class="wtc_roster-simple-table-right-plan-roster-wrap"
                    v-for="(planRoster, index1) in rosterData.planRosterList" :key="index1"
                    :attFileId="rosterData.rosterPerson.id" :rowNum="index*2" :rowIndex="index" :colIndex="index1"
                    :cellRoster="planRoster" :lockPower="allRosterData.lockPower" :savePower="allRosterData.savePower"
                    :cellType="'plan'">
                </cell>
            </ul>
            <ul class="wtc_roster-simple-table-right-actual-list"
                v-show="allRosterData.showType === 'SHOW_ALL' || allRosterData.showType === 'SHOW_REAL'">
                <cell @showRightClickMenu="showRightClickMenu" @shiftActiveCells="shiftActiveCells"
                    @cancelAllActive="cancelAllActive" ref="cells"
                    class="wtc_roster-simple-table-right-actual-roster-wrap"
                    v-for="(actualRoster, index1) in rosterData.realRosterList" :key="index1"
                    :attFileId="rosterData.rosterPerson.id" :rowNum="index*2+1" :rowIndex="index" :colIndex="index1"
                    :cellRoster="actualRoster" :lockPower="allRosterData.lockPower" :savePower="allRosterData.savePower"
                    :cellType="'actual'">
                </cell>
            </ul>
        </li>
        ...
        methods: {
            ...
            * */
            ...
            keydownHandle(e) {
                ...
                if (!e.ctrlKey && e.shiftKey) {
                    $this.isShift = true;
                    //todo $($this.rootDom).css('userSelect', 'none'); --- 这样写严重影响性能
                }
            ...
            shiftActiveCells() {
                ...
                this.$refs.cells.forEach((cell, index) => {
                    // 先判断是否在行列矩阵内：
            ...
            * */
            showRightClickMenu(positionX, positionY) {
               ...
               <div class="wtc_roster_rightClickMenu-item" style="padding:10px;color:#333;min-width: 200px;height: 259px;background:#f2f2f2;box-sizing: border-box;cursor: auto;overflow: hidden;">
                    <div class="wtc_roster_rightClickMenu-input-wrap" style="display:flex;width: 100%;background: #fff;border: 1px solid #eee;margin-bottom: 10px;padding:6px;;box-sizing: border-box">
                        <i class="wtc_roster_rightClickMenu-input-icon kdfont kdfont-sousuo2" style="flex:0;font-size: 14px;color:#999;margin-right: 4px;vertical-align:sub;"></i>
                        <input type="text" class="wtc_roster_rightClickMenu-input" style="flex:1;outline: none;border: none;;color:#333;font-size: 12px;" placeholder="${this.KDApi.getLangMsg(this.model, 'placeholder1')}">
                    </div>
                ...
                let menuData = {
                    ...
                    data: [
                        [  // 组
                            ...
                            {
                                value: this.KDApi.getLangMsg(this.model, 'planComplete'),
                                iconLeft: 'kdfont kdfont-jiesuo3',
                                iconRight: '',
                                disabled: objDisabled.planComplete,
                                index: 7,
                                isHtml: false,
                                children: []
                            },
            ...
            * */
            rightMenuDisabled() {
                ...
                if (allActual) {
                    objDisabled.planComplete = true;  // 完成计划排班按钮置灰
                }
                ...
                if (!this.allRosterData.completePower) objDisabled.planComplete = true;  // 无完成计划排班的权限权限，则完成计划排班按钮置灰
            ...
            * */
            refreshMenuShiftList(shiftList) {
                ...
                for (let i = 0; i < shiftInfo.length; i++) {
                    ...
                    shiftStr += `
                        <li class="wtc_roster_rightClickMenu-shift-item" style="position:relative;display:flex;padding:4px 0;cursor: pointer" shiftId="${shiftInfo[i].id}" shiftBoId="${shiftInfo[i].boId}">
                            ...
                            <div class="wtc_roster_rightClickMenu-item-text-wrap" title="${title}" style="flex:1;font-size: 12px;color:#333;white-space: nowrap;">
                                <div class="wtc_roster_rightClickMenu-item-number" style="display:inline-block;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;max-width:64px;">${shiftInfo[i].number}</div>
                                <div class="wtc_roster_rightClickMenu-item-text" style="display:inline-block;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;max-width:120px;"> | ${shiftInfo[i].name}</div>
                            </div>
                            <!--<div class="wtc_roster_rightClickMenu-item-line" style="flex:0 0 1px;"></div>-->
                        </li>
                    `;
                }
                ...
                // let $dom_shiftItem = $('.wtc_roster_rightClickMenu-shift-item', $dom_shiftList);
                // $dom_shiftItem.on('mouseenter', function (event) {
                //     // $('.wtc_roster_rightClickMenu-item-time-list', this).css('display', 'block');
                //     $this.showShiftTimeDetail(this.querySelector('.wtc_roster_rightClickMenu-item-line'), $(this).attr('shiftId'))
                // }).on('mouseleave', function () {
                //     // $('.wtc_roster_rightClickMenu-item-time-list', this).css('display', 'none');
                //     $this.hideShiftTimeDetail();
                // });
                this.menuInputMatch();
            },
            ...
            * */
            getFilterShiftList(shiftList, curActiveCell) {
                ...
                for (let i = 0; i < shiftList.length; i++) {
                    let endBsled = shiftList[i].endBsled || '2999-12-31 00:00:00';
                    if (shiftList[i].firstBsed <= curActiveCell.rosterDate && endBsled >= curActiveCell.rosterDate) {
            ...
            * 用户滑入班次中显示更多icon时，触发
            * */
            showShiftTimeDetail(target, shiftId) {
                let $this = this;
                $this.myTool.popover_isEnter = true;

                setTimeout(function () {
                    if ($this.myTool.popover_isEnter && !document.querySelector('.wtc_kztTool_popover-wrap')) {
                        for (let i = 0; i < $this.arrInsertShift.length; i++) {
                            if ($this.arrInsertShift[i].id === shiftId && $this.arrInsertShift[i].shiftEntryList) {
                                let str = '';
                                for (let n = 0; n < $this.arrInsertShift[i].shiftEntryList.length; n++) {
                                    let objShiftEntry = $this.arrInsertShift[i].shiftEntryList[n];
                                    let outWorkType = null;
                                    switch (objShiftEntry.outWorkType) {
                                        case 'W':
                                            outWorkType = $this.KDApi.getLangMsg($this.model, 'work');
                                    ...
                                    str += `
                            <div class="wtc_roster-simple-table-shift-detail-time-item" style="margin-bottom: 6px;white-space: nowrap;">${outWorkType}：${objShiftEntry.shiftStartDateStr} ~ ${objShiftEntry.shiftEndDateStr}（${objShiftEntry.workTime}h）</div>
                        `;
                                }
            ...
            insertShift_event(shiftId, shiftBoId, type) {
                ...
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    ...
                    if (cell.active && !cell.cellRoster.lock) {
                        ...
                        arrIsActual.push(cell.cellType === 'actual');
                ...
                // 修改数据：
                this.getRosterData(type, arrRowIndex, arrColIndex, arrShiftId, arrShiftBoId, arrIsActual);
           ...
           * */
            delShift_event() {
                ...
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    ...
                    if (cell.active && !cell.cellRoster.lock && cell.cellRoster.shiftId && cell.cellType !== 'plan') {
                        arrRowIndex.push(cell.rowIndex);
                        arrColIndex.push(cell.colIndex);
                        arrIsActual.push(cell.cellType !== 'plan');
                    }
                }

                // 修改数据：
                this.getRosterData('del', arrRowIndex, arrColIndex, arrShiftId, arrShiftBoId, arrIsActual);
            ...
            * */
            lockShift_event() {
                ...
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    ...
                    if (!cell.active || cell.cellRoster.lock || !this.allRosterData.lockPower && cell.cellType !== 'actual') continue;
                ...
                // 修改数据：
                this.getRosterData('lock', arrRowIndex, arrColIndex, arrShiftId, arrShiftBoId, arrIsActual);
            ...
            * */
            unlockShift_event() {
                ...
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    ...
                    if (!cell.active || !cell.cellRoster.lock || !this.allRosterData.unlockPower && cell.cellType !== 'actual') continue;
                ...
                // 修改数据：
                this.getRosterData('unlock', arrRowIndex, arrColIndex, arrShiftId, arrShiftBoId, arrIsActual);
            ...
            * */
            planComplete_event() {
                ...
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    ...
                    // 判断是否有权限（只针对存在班次的计划班）
                    if (cell.active && this.allRosterData.completePower && cell.cellType === 'plan' && cell.cellRoster.shiftId) {
                        arrRowIndex.push(cell.rowIndex);
                        arrColIndex.push(cell.colIndex);
                        arrIsActual.push(cell.cellType === 'actual');
                    }
                ...
                // 修改数据：
                this.getRosterData('planComplete', arrRowIndex, arrColIndex, arrShiftId, arrShiftBoId, arrIsActual);
            ...
            * 插入班次
            * */
            insertShift(shiftId, shiftBoId, arrDateType) {
                let dateTypeIndex = 0;
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    let cell = this.$refs.cells[i];
                    if (cell.active && !cell.cellRoster.lock) {
                        this.refreshRosterData('insert', cell, shiftId, shiftBoId, arrDateType[dateTypeIndex]);
                        dateTypeIndex++;
            ...
            * 删除班次
            * */
            delShift(arrDateType) {
                let dateTypeIndex = 0;
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    let cell = this.$refs.cells[i];
                    // 1.当该单元格存在班次，且没有被锁；2.计划班不允许删
                    if (cell.active && !cell.cellRoster.lock && cell.cellRoster.shiftId && cell.cellType !== 'plan') {
                        this.refreshRosterData('del', cell, null, null, arrDateType[dateTypeIndex]);
                        dateTypeIndex++;
          ...
          * */
            ...
            * */
            planComplete() {
                for (let i = 0; i < this.$refs.cells.length; i++) {
                    ...
                    // 判断是否有权限（只针对存在班次的计划班）
                    if (cell.active && this.allRosterData.completePower && cell.cellType === 'plan' && cell.cellRoster.shiftId) {
                        this.refreshRosterData('planComplete', cell, null, null);
                    }
                }
            },
            ...
            * */
            ...
            /*
            * 用户操作后，同步最新数据到rosterData（人员）
            * */
            getRosterData(type, arrRowIndex, arrColIndex, arrShiftId, arrShiftBoId, arrIsActual) {
                let arrPara = [];

                for (let i = 0; i < arrRowIndex.length; i++) {
                    let rosterList;
                    let opType = null;
                    let attFileId = this.allRosterData.rosterShiftModelList[arrRowIndex[i]].rosterPerson.id;
                    let attPersonId = this.allRosterData.rosterShiftModelList[arrRowIndex[i]].rosterPerson.attPersonId;

                    if (!arrIsActual[i]) {
                        rosterList = this.allRosterData.rosterShiftModelList[arrRowIndex[i]].planRosterList;
                    } else {
                        rosterList = this.allRosterData.rosterShiftModelList[arrRowIndex[i]].realRosterList;
                    }
                    ...
                    switch (type) {
                        ...
                        case 'planComplete':
                            ...
                            objRoster_copy.planComplete = true;
                            opType = '7';
                            break;
                    }
            ...
            * 用户操作后，同步最新数据到rosterData（人员）
            * */
            refreshRosterData(type, cell, shiftId, shiftBoId, dateType) {
                switch (type) {
                    case 'insert':
                    case 'parse':
                        //需要替换原数据中的saveType和shiftModel的值
                        ...
                        // let dateAttribute = this.allRosterData.rosterShiftModelList[cell.rowIndex].planRosterList[cell.colIndex].dateAttribute;
                        // this.$set(cell.cellRoster, "dateAttribute", dateAttribute);
                        this.$set(cell.cellRoster, 'shiftId', shiftId);
                        this.$set(cell.cellRoster, "shiftBoId", shiftBoId);
                        if (dateType) {
                            this.$set(this.allRosterData.rosterShiftModelList[cell.rowIndex].planRosterList[cell.colIndex], "dateAttribute", dateType.dateAttribute);
                            this.$set(this.allRosterData.rosterShiftModelList[cell.rowIndex].realRosterList[cell.colIndex], "dateAttribute", dateType.dateAttribute);
                            this.allRosterData.rosterShiftModelList[cell.rowIndex].planRosterList[cell.colIndex].dateType = dateType;
                            this.allRosterData.rosterShiftModelList[cell.rowIndex].realRosterList[cell.colIndex].dateType = dateType;
                        }
                        break;
                    case 'del':
                        if (cell.cellRoster.hasRoster === false) {
                    ...
                    case 'planComplete':
                        ...
                        this.$set(cell.cellRoster, "planComplete", true);
                        cell.active = false;
                        break;
                }
            },
            ...
            opPower(args) {
                if (args.power) {
                    switch (args.opType) {
                        case '1':  //插入
                            this.insertShift(this.insertShiftId, this.insertShiftBoId, args.opDateType || []);
                            break;
                        case '2':  //复制
                            this.copyShift();
                            break;
                        case '3':  //黏贴
                            this.insertShift(this.copyShiftId, this.copyShiftBoId, args.opDateType || []);
                            break;
                        case '4':  //删除
                            this.delShift(args.opDateType || []);
                            break;
                        case '5':  //锁定
                            this.lockShift();
                            break;
            ...
            timeOverlap(arrOverlapRoster) {
                ...
                for (let j = 0; j < this.$refs.cells.length; j++) {
                    ...
                    for (let i = 0; i < arrOverlapRoster.length; i++) {
                        if (cell.attFileId === arrOverlapRoster[i].attFileId && cell.cellRoster.rosterType === arrOverlapRoster[i].rosterType && cell.cellRoster.rosterDateStr === arrOverlapRoster[i].rosterDate) {
...
<style scoped>
    .wtc_roster-simple-table-right-group-list {
        display: inline-block;
        background-color: #fff;
        box-sizing: border-box;
        z-index: 5;
        vertical-align: top;
        /*width: calc(((100% - 90px) - 40px) - 0px);*/
        overflow: hidden;
    }

    .wtc_roster-simple-table-right-group-list .wtc_roster-simple-table-right-group-row {
        display: flex;
        box-sizing: border-box;
        height: 30px;
    }

    .wtc_roster-simple-table-right-group-list .wtc_roster-simple-table-right-group-row .wtc_roster-simple-table-right-group-roster-wrap {
        flex: 1;
        height: 30px;
        min-width: 48px;
    ...
    .wtc_roster-simple-table-right-data-item {
        display: flex;
        flex-direction: column;
        justify-content: center;
    }

    .wtc_roster-simple-table-right-group-list .wtc_roster-simple-table-right-data-item>ul {
        display: flex;
    }

    .wtc_roster-simple-table-right-group-list .wtc_roster-simple-table-right-data-item .wtc_roster-simple-table-right-plan-list {
        box-sizing: border-box;
        height: 30px;
    }

    .wtc_roster-simple-table-right-actual-list {
        height: 30px;
    }

    .wtc_roster-simple-table-right-data-item .wtc_roster-simple-table-right-actual-list>li {
        border-bottom: 1px solid #D9D9D9;
    }

    .wtc_roster-simple-table-right-actual-list .wtc_roster-simple-table-right-actual-roster-wrap {
        border-bottom: 1px solid #d9d9d9;
    }
</style>