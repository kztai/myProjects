(function (window, factory) {
    ...
    KdWtcTimeCard.prototype.init = function(){
        ...
        $(this.rootDom).mousemove(function (event) {
            if($this.moveFlag){
                let curHeight = $this.dataTotalHeight + ($this.startY - event.clientY);
                // 5为拖动线的高度，防止被隐藏：
                // 28为底部展开按钮的高度：
    ...
    KdWtcTimeCard.prototype.init_cardRecord = function(){
        ...
        if(this.data_cardRecord[0]){
            for (let j = 0; j < this.data_cardRecord[0].length; j++) {
                let col = 'col-' + (j + 1);
                curStr += `<div class="wtc_timeCard_card-record-item-col item-${col}">`;

                for (let i = 0; i < this.data_cardRecord.length; i++) {
                    let row = 'row-' + (i + 1);
                    ...
                    if(j === 0) {
                        ...
                        if (this.isFirstItem(i, j)) {
                            ...
                            if(this.data_cardRecord[i][j].abnormal){  // 异常的打卡记录
    ...
    KdWtcTimeCard.prototype.init_attendanceTotal = function(){
        ...
        if(this.data_attendanceTotal[0]){
            for (let i = 0; i < this.data_attendanceTotal.length; i++) {
    ...
    KdWtcTimeCard.prototype.init_operationLog = function(){
        ...
        if(this.data_operationLog[0]){
            for (let i = 0; i < this.data_operationLog.length; i++) {
                let row = 'log-row-' + (i+1);
                curStr += `<div class="wtc_timeCard_total-main-log-row ${row}">`;
                for (let j = 0; j < this.data_operationLog[i].length; j++) {
                    let col = 'log-col-' + (j+1);
    ...
    * */
    KdWtcTimeCard.prototype.init_dynamicEvent = function(){
        ...
        this.dom_btnMore.click(function (event) {
            let curPop = this.parentElement.lastElementChild;
    ...
    KdWtcTimeCard.prototype.dateSelectAll = function(type) {
        // 1是全展开，2是全收起：
        for(let i = 0; i < this.dom_col1.length; i++){
            if(type === 2){
    ...
    KdWtcTimeCard.prototype.toggleDateSelected = function(target, type){
        ...
        let arrClass = target.className.split(' ');
        let arrRow = [];
        let openRow = 0;  // 为正数表示展开行（加高度），负数表示折叠的行（减高度）
        ...
        for(let i = 0; i < arrRow.length; i++){
            ...
            if(i > 0){
                let $curRow = $('.' + arrRow[i], this.rootDom);
                // 1是下拉，2是上拉，3是展开异常项：
                if(type === 1) {
                    if($($curRow[1]).css('display') === 'none') openRow++;
                    $curRow.css('display', 'flex');
                } else if(type === 2) {
                    if($($curRow[1]).css('display') === 'flex') openRow--;
                    $curRow.css('display', 'none');
                } else {
                    if($curRow.hasClass('item-abnormal')) {
                        if($($curRow[1]).css('display') === 'none') openRow++;
                        $curRow.css('display', 'flex');
                    } else {
                        if($($curRow[1]).css('display') === 'flex') openRow--;
